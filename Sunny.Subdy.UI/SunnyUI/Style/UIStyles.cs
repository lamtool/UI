...
using static Sunny.UI.UIStyles;
...
namespace Sunny.UI
{
    ...
    public static class UIStyles
    {
        ...
        // [DynamicDependency("LOGFONT", "Sunny.UI.UIStyles")]
        internal static byte GetGdiCharSet(string fontName)
        {
            if (FontCharSets.ContainsKey(fontName))
                return FontCharSets[fontName];

            using Font font = new Font(fontName, 16);

            // Gọi hàm tự viết để lấy LOGFONT
            LOGFONT obj = NativeMethods.FontToLogFont(font);
    ...
    internal static class NativeMethods
    {
        ...
        public static LOGFONT FontToLogFont(Font font)
        {
            // Tạo HFONT từ Font .NET
            IntPtr hFont = font.ToHfont();
            try
            {
                LOGFONT logfont;
                // Lấy kích cỡ của struct LOGFONT
                int size = Marshal.SizeOf<LOGFONT>();

                // Gọi GetObject để copy dữ liệu LOGFONT từ HFONT
                int result = GetObject(hFont, size, out logfont);
                if (result == 0)
                {
