...
namespace AutoAndroid
{
    ...
    {
        ADBClient service;
        private readonly string path_MaxChange = AppDomain.CurrentDomain.BaseDirectory + "Android\\App\\DTAChange.apk";
        private readonly string path_DeviceInfoHW = AppDomain.CurrentDomain.BaseDirectory + "Android\\App\\DeviceInfoHW.apk";
        public static string package_MaxChange = "com.dtasoftware.dtachanger";
        ...
        public void Open()
        {
            for (int i = 0; i < 5; i++)
            {
                service.LogHelper.SUCCESS("Đang mở ứng dụng DTAChange");
                service.AppStart(package_MaxChange, true, true, wait: true);
                service.SetSize();
                if (service.AppWait(package_MaxChange))
                {
                    service.LogHelper.SUCCESS("Đã mở ứng dụng DTAChange");
                    break;
                }
            }
        ...
        public bool Install()
        {
            service.LogHelper.SUCCESS("Cài đặt DTAChange");
            List<string> list = service.AppList();
            if (list.Contains(package_MaxChange))
            {
                service.UninstallApp(package_MaxChange);
            }
            for (int i = 0; i < 5; i++)
            {
            ...
            service.Shell("pm grant com.dtasoftware.dtachanger android.permission.READ_EXTERNAL_STORAGE");
            service.Shell("pm grant com.dtasoftware.dtachanger android.permission.WRITE_EXTERNAL_STORAGE");
            SetEnableModule();
        ...
        public bool Change(string pathDevice, bool backup)
        {
            bool flag = false;
        ...
        private bool Restore(string pathDevice)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                string text2 = service.Shell($" su -c cp /sdcard/{fileName} /data/data/com.dtasoftware.dtachanger/{fileName}");
                text2 = service.Shell($" su -c tar -xzvf /data/data/com.dtasoftware.dtachanger/{fileName}");
                text2 = service.Shell(" su -c \"ls -l /data/data | grep com.dtasoftware.dtachanger | awk '{print $3\\\":\\\"$4}'\"");
                flag = text2 != "";
                text2 = service.Shell(" su -c chown -R " + text2 + " /data/data/com.dtasoftware.dtachanger");
                if (!flag)
                {
                    Thread.Sleep(2000);
                    continue;
        ...
        private string GetInfoDeviceName(int waitTimeInSeconds = 0)
        {
            ...
            do
            {
                ...
                // which located within the shared_prefs directory of the com.dtasoftware.dtachanger application.
                string fileContent = service.Shell(" su -c 'cat /data/data/com.dtasoftware.dtachanger/shared_prefs/Device.xml'");
        ...
        private bool ChangeDeviceName()
        {
            ...
            try
            {
                ...
                string text2 = "am broadcast -a com.dtasoftware.dtachanger.CHANGE -n com.dtasoftware.dtachanger/.AdbCaller --es brand xiaomi --ez on true";
        ...
        private bool BackupDeviceInfoChange(string pathDevice)
        {
            ...
            for (int i = 0; i < 10; i++)
            {
                service.Shell($" su -c tar -czvf /data/data/com.dtasoftware.dtachanger/{fileName} /data/data/com.dtasoftware.dtachanger/shared_prefs/*");
                service.Shell($" su -c cp /data/data/com.dtasoftware.dtachanger/{fileName} /sdcard/{fileName}");
                ProcessHelper.RunAdbWithTimeout($" -s {service.Device.Serial} pull /sdcard/{fileName} \"" + pathDevice + "\"");
                service.Shell(" su -c rm -rf /data/data/com.dtasoftware.dtachanger/*.tar.gz");
        ...
        public bool CheckInternet()
        {
            ...
            try
            {
                ...
                service.Shell(" pm grant com.dtasoftware.dtachanger android.permission.READ_EXTERNAL_STORAGE");
                service.Shell(" pm grant com.dtasoftware.dtachanger android.permission.WRITE_EXTERNAL_STORAGE");
                string output = service.Shell(" am broadcast -a com.dtasoftware.dtachanger.GET_DEVICE_IP -n com.dtasoftware.dtachanger/.NetworkUtils");
                if (string.IsNullOrEmpty(output) || !output.Contains("data"))
                {
                    return false;
                }
                string pattern = @"data=""(.*?)""";
                ...
                if (match.Success)
                {
                    ...
                    service.LogHelper.SUCCESS($"IP: {dataValue}");
                    return true;
                }
            }
            catch (Exception ex)
        ...
        {
            string[] requiredApps = { "com.dtasoftware.dtachanger", "com.facebook.katana", "ru.andr7e.deviceinfohw" };
        ...
        public string CheckIP()
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                try
                {
                    ...
                    string result = service.Shell("am broadcast -a com.dtasoftware.dtachanger.GET_DEVICE_IP -n com.dtasoftware.dtachanger/.AdbCaller");
                    ...
                    if (match.Success)
                    {
                        ...
                        ip = $"IP: {data["ip"]}, Country: {data["country"]}, CC: {data["cc"]}";
                        service.LogHelper.SUCCESS(ip);
                        return ip;
                    }
                }
                catch
